...
interface BarraAcaoSelecionadosProps {
  quantidade: number;
  totalCentavos: number;
  onDarBaixa: () => void;
}

export default function BarraAcaoSelecionados({
  quantidade,
  totalCentavos,
  onDarBaixa,
}: BarraAcaoSelecionadosProps) {
  return (
    <div className="sticky bottom-0 bg-white border-t border-gray-200 shadow-lg px-4 py-3 flex items-center justify-between gap-4 flex-wrap">
      ...
      {quantidade > 0 && (
        <Button onClick={onDarBaixa} variant="primary">
          Dar Baixa Em {quantidade} {quantidade === 1 ? "Venda" : "Vendas"}
        </Button>
      )}
    </div>
  );
