...
export type OrdenarPor = "data" | "numero";
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex gap-1">
          <select
            ...
            onChange={(e) =>
              onChange({ ...filtros, ordenarPor: e.target.value as OrdenarPor })
            }
            className="border border-gray-300 rounded-md px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500"
          >
            <option value="data">Data</option>
            <option value="numero">Número da venda</option>
          </select>
          <button
            type="button"
            onClick={() =>
              onChange({
                ...filtros,
                ordenarDirecao: filtros.ordenarDirecao === "asc" ? "desc" : "asc",
              })
            }
            title={filtros.ordenarDirecao === "asc" ? "Crescente" : "Decrescente"}
            aria-label="Inverter direção da ordenação"
            className="border border-gray-300 rounded-md px-2 text-sm text-gray-600 hover:bg-gray-50"
          >
            {filtros.ordenarDirecao === "asc" ? "↑" : "↓"}
          </button>
        </div>
      </div>
    </div>
