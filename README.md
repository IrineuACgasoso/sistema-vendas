...
Sistema interno de cadastro, acompanhamento, edição e baixa de vendas, com
autenticação simples de usuário único, banco de dados Firebase Firestore e
deploy na Vercel.
...
│   ├── login/, menu/, cadastro/, vendas/, baixa/
│   └── actions/          → auth, pessoas, vendas (isoladas por domínio)
├── components/
│   ├── ui/                → componentes genéricos reutilizáveis (inclui DateInput
│   │                          segmentado dd/mm/aaaa)
│   ├── login/, menu/, cadastro/, vendas/, baixa/
├── lib/
│   ├── firebase/admin.ts  → conexão com Firestore (server-only)
│   ├── auth/               → sessão, senha, rate limit
│   ├── validation/          → schemas Zod
│   └── utils/                → data, moeda, texto (capitalização), ordenação, buscas
...
Para o detalhamento completo (arquivo a arquivo) e as regras de negócio,
veja [`ORGANIZATION.md`](./ORGANIZATION.md).
...
**Menu**: escolha entre Cadastrar, Vendas e Baixa.
...
(`pessoas`), alimentam autocomplete por prefixo e são auto-capitalizados
por palavra ao digitar. O campo Data usa três segmentos independentes
(dd/mm/aaaa) e sempre começa em branco, inclusive após cada venda cadastrada.

**Vendas**: lista todas as vendas (abertas e fechadas), com filtro por
período, nome e tipo de pagamento (Pix/Depósito/Transferência/Promissórias —
essas últimas nunca se misturam com uma forma específica) e ordenação por
data ou número. Um número de venda pode se repetir entre lançamentos (ex:
Pix parcelado) — não há checagem de duplicidade. Botão "Selecionar todas"
marca de uma vez as vendas que já podem ser fechadas. A tela sincroniza
sozinha (polling a cada 5s) quando outra aba cadastra uma venda nova, sem
precisar de F5. Dois modos extras, ativados pelos ícones no cabeçalho:
- **Editar**: clique numa venda aberta para editar nome, valor e número inline.
- **Excluir**: seleciona vendas e apaga permanentemente após confirmar com
  senha — deixado claro na UI que isso NÃO é dar baixa.

**Baixa**: mesmos filtros da tela de Vendas, mas só vendas já fechadas;
seleção múltipla com checkbox; total dos selecionados sempre visível;
confirmação em duas etapas (diálogo + senha novamente) antes de baixar
(excluir definitivamente, sem histórico) as vendas selecionadas.
...
- O filtro por nome, por tipo de pagamento (excluindo promissórias) e a
  ordenação são aplicados em memória após a busca no Firestore, o que é
  aceitável para o volume esperado deste sistema. Se o volume crescer muito,
  considerar Algolia/Typesense no futuro.
- `tailwind.config.js` inclui `src/lib/**` no `content` — as classes de cor
  usadas em `formaPagamento.ts` (fora de `app/`/`components/`) precisam
  disso pra serem geradas pelo Tailwind.
- Configs duplicadas foram removidas: só existe `next.config.js` (não
  `.mjs`/`.ts`), `postcss.config.js` (não `.mjs`, que referenciava um plugin
  não instalado) e `.eslintrc.json` (não `eslint.config.mjs`, formato flat
  incompatível com a versão instalada do ESLint).